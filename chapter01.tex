...
sides. Use option \kwl{http-server-close} to preserve client persistent connections
while handling every incoming request individually, dispatching them one after
another to servers, in HTTP close mode. Use option \kwl{httpclose} to switch both
sides to HTTP close mode. The option \kwl{forceclose} and option \kwl{http-pretend-keepalive} help working around servers misbehaving in HTTP close
mode.
...
\index{Relative URI}
    ...
    \site{/serv/login.php?lang=en\&profile=2}
...
\index{Absolute URI}
   ...
   \site{http://192.168.0.12:8080/serv/login.php?lang=en\&profile=2}
...
define a total of 3 values for the \httphdr{Accept:} header.
...
codes \httpcode{1xx}. These messages are special in that they don't convey any part of the
response, they're just used as sort of a signaling message to ask a client to
continue to post its request for instance. In the case of a status \httpcode{100} response
the requested information will be carried by the next non-\httpcode{100} response message
following the informational one. This implies that multiple responses may be
sent to a single request, and that this only works when keep-alive is enabled
(\httpcode{1xx} messages are HTTP/1.1 only). HAProxy handles these messages and is able to
correctly forward and skip them, and only process the next \httpcode{non-100} response. As
such, these messages are neither logged nor transformed, unless explicitly
state otherwise. Status \httpcode{101} messages indicate that the protocol is changing
...
messages. It can be composed of one or multiple words, such as \httpstatus{OK}, \httpstatus{Found},
or \httpstatus{Authentication Required}.
...
\item[403] when a request is forbidden by a \kwl{block} ACL or \kwl{reqdeny} filter
...
\item[502] when the server returns an empty, invalid or incomplete response, or
        when an \kwl{rspdeny} filter blocks the response.
\item[503] when no server was available to handle the request, or in response to
        monitoring requests which match the \kwl{monitor fail} condition
\item[504] when the response timeout strikes before the server responds
\end{description}

The error \httpcode{4xx} and \httpcode{5xx} codes above may be customized (see \kwl{errorloc} in section~\ref{sec:keywords_reference}).
