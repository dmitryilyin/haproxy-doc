...
\chr{-} (dash), \chr{\_} (underscore) , \chr{.} (dot) and \chr{:} (colon). ACL names are
...
\subsubsection[acl]{\kw{acl} <aclname> <criterion> [flags] [operator] <value> ...}
\index{acl}
...
\subsubsection[appsession]{\kw{appsession} <cookie> len <length> timeout <holdtime> [request-learn] [prefix] [mode <path-parameters|query-string>]}
\index{appsession}
...
See also: \kwl{cookie}, \kwl{capture cookie}, \kwl{balance}, \kwl{stick},
  \kwl{stick-table}, \kwl{ignore-persist}, \kwl{nbproc} and \kwl{bind-process}.

\subsubsection[backlog]{\kw{backlog} <conns>}
\index{backlog}
...
See also: \kwl{maxconn} and the target operating system's tuning guide.

\subsubsection[balance] {\kw{balance} <algorithm> [ <arguments> ]}
  ...
  Arguments:
    \paragraph*{<algorithm>}
      ...
      \item[url\_param]
                  ...
                  \chr{?} in the URL. Optionally, specify a number of octets to
...
See also: the \kwl{rdp\_cookie} pattern fetch function.
       \end{description}
...
See also: \kwl{dispatch}, \kwl{cookie}, \kwl{appsession}, \kwl{transparent},
   \kwl{hash-type} and \kwl{http\_proxy}.

\subsubsection[bind]{\kw{bind} [<address>]:<port\_range> [, ...]}
...
\index{bind}
  ...
  \item[<user>]
                  ...
                  section's \kwl{unix-bind} statement. Note that some platforms
                  simply ignore this.

  \item[<group>]
                  is the name of a group that will be used to create the UNIX
                  socket. It can also be set by default in the global section's
                  \kwl{unix-bind} statement. Note that some platforms simply ignore
                  this.
  ...
  \item[accept-proxy]
                  is an optional keyword which enforces use of the PROXY
...
See also: \kwl{source}, \kwl{option forwardfor}, \kwl{unix-bind} and the PROXY protocol
             documentation.

\subsubsection[bind-process]{\kw{bind-process} [ all | odd | even | <number 1-32> ] ...}
\index{bind-process}
...
See also: \kwl{nbproc} in global section.

\subsubsection[block]{\kw{block} { if | unless } <condition>}
\index{block}
...
\subsubsection[capture cookie]{\kw{capture cookie} <name> len <length>}
...
See also: \kwl{capture request header}, \kwl{capture response header} as well as
            chapter~\ref{chap:logging} about logging.

\subsubsection[capture request header]{\kw{capture request header} <name> len <length>}
...
See also: \kwl{capture cookie}, \kwl{capture response header} as well as chapter~\ref{chap:logging}
             about logging.

\subsubsection[capture response header]{\kw{capture response header} <name> len <length>}
...
See also: \kwl{capture cookie}, \kwl{capture request header} as well as chapter~\ref{chap:logging}
             about logging.

\subsubsection[clitimeout]{\kw{clitimeout} <timeout> (deprecated)}
\index{clitimeout}
  ...
  Please use \kwl{timeout client} instead.


See also: \kwl{timeout client}, \kwl{timeout http-request}, \kwl{timeout server}, and
            \kwl{srvtimeout}.

\subsubsection[contimeout]{\kw{contimeout} <timeout> (deprecated)}
\index{contimeout}
  ...
  deprecated. Please use \kwl{timeout connect}, \kwl{timeout queue} or \kwl{timeout tarpit}
  instead.


See also: \kwl{timeout connect}, \kwl{timeout queue}, \kwl{timeout tarpit},
             \kwl{timeout server}, \kwl{contimeout}.


\subsubsection[cookie]{\kw{cookie} <name> [ rewrite | insert | prefix ] [ indirect ]  [ nocache ] [ postonly ] [ preserve ] [ httponly ] [ secure ] [ domain <domain> ]* [ maxidle <idle> ] [ maxlife <life> ]}
\index{cookie}
...
See also: the \kwl{insert} and \kwl{postonly} options.
...
See also: the \kwl{insert} and \kwl{nocache} options.
...
See also: \kwl{appsession}, \kwl{balance source}, \kwl{capture cookie}, \kwl{server}
             and \kwl{gnore-persist}.

\subsubsection[default-server]{\kw{default-server} [param*]}
...
See also: \kwl{server} and chapter~\ref{chap:server_options} about server options

\subsubsection[default\_backend]{\kw{default\_backend} <backend>}
\index{default\_backend}
  ...
  \kwl{use\_backend} keyword, it is often useful to indicate which backend will be
...
See also: \kwl{use\_backend}, \kwl{reqsetbe}, \kwl{reqisetbe}
...
\index{disabled}
  ...
  The \kwl{disabled} keyword is used to disable an instance, mainly in order to
...
See also: \kwl{enabled}

\subsubsection[dispatch]{\kw{dispatch} <address>:<port>}
\index{dispatch}
  ...
  The \kwl{dispatch} keyword designates a default server for use when no other
...
\subsubsection[enabled]{\kw{enabled}}
\index{enabled}
...
See also: \kwl{disabled}

\subsubsection[errorfile]{\kw{errorfile} <code> <file>}
\index{errorfile}
...
See also: \kwl{errorloc}, \kwl{errorloc302}, \kwl{errorloc303}
...
\subsubsection[errorloc]{\kw{errorloc} <code> <url>}
\subsubsection[errorloc302]{\kw{errorloc302} <code> <url>}
\index{errorloc}
\index{errorloc302}
  ...
  Code 200 is emitted in response to requests matching a \kwl{monitor-uri} rule.

  Note that both keyword return the HTTP 302 status code, which tells the
  client to fetch the designated URL using the same HTTP method. This can be
  quite problematic in case of non-GET methods such as POST, because the URL
  sent to the client might not be allowed for something other than GET. To
  workaround this problem, please use \kwl{errorloc303} which send the HTTP 303
  status code, indicating to the client that the URL must be fetched with a GET
  request.


See also: \kwl{errorfile}, \kwl{errorloc303}

\subsubsection[errorloc303]{\kw{errorloc303} <code> <url>}
\index{errorloc303}
...
See also: \kwl{errorfile}, \kwl{errorloc}, \kwl{errorloc302}

\subsubsection[force-persist]{\kw{force-persist} { if | unless } <condition>}
\index{force-persist}
...
See also: \kwl{option redispatch}, \kwl{ignore-persist}, \kwl{persist},
             and chapter~\ref{chap:using_acl} about ACL usage.

\subsubsection[fullconn]{\kw{fullconn} <conns>}
\index{fullconn}
  ...
  When a server has a \kwl{maxconn} parameter specified, it means that its number
  of concurrent connections will never go higher. Additionally, if it has a
  \kwl{minconn} parameter, it indicates a dynamic limit following the backend's
  load. The server will then always accept at least \kwl{minconn} connections,
  never more than \kwl{maxconn}, and the limit will be on the ramp between both
  values when the backend has less than \kwl{conns} concurrent connections. This
...
See also: \kwl{maxconn}, \kwl{server}

\subsubsection[grace]{\kw{grace} <time>}
\index{grace}
...
\subsubsection[hash-type]{\kw{hash-type} <method>}
\index{hash-type}
...
See also: \kwl{balance}, \kwl{server}

\subsubsection[http-check disable-on-404]{\kw{http-check disable-on-404}}
\index{http-check disable-on-404}
  ...
  option only works in conjunction with the \kwl{httpchk} option. If this option
  is used with \kwl{http-check expect}, then it has precedence over it so that 404
  responses will still be considered as soft-stop.


See also: \kwl{option httpchk}, \kwl{http-check expect}

\subsubsection[http-check expect]{\kw{http-check expect} [!] <match> <pattern>}
\index{http-check expect}
  ...
  By default, \kwl{option httpchk} considers that response statuses 2xx and 3xx
  are valid, and that others are invalid. When \kwl{http-check expect} is used,
  it defines what is considered valid or invalid. Only one \kwl{http-check}
  ...
  defined by the global \kwl{tune.chksize} option, which defaults to 16384 bytes.
  ...
  Last, if \kwl{http-check expect} is combined with \kwl{http-check disable-on-404},
...
See also: \kwl{httpchk}, \kwl{http-check disable-on-404}

\subsubsection[http-check send-state]{\kw{http-check send-state}}
...
See also: \kwl{option httpchk}, \kwl{http-check disable-on-404}

\subsubsection[http-request]{\kw{http-request} \{ allow | deny | auth [realm <realm>] \} [ \{ if | unless \} <condition> ]}
\index{http-request}
...
See also: \kwl{stats http-request}, section~\ref{sec:userlists} about userlists and chapter~\ref{chap:using_acl}
             about ACL usage.

\subsubsection[http-send-name-header]{\kw{http-send-name-header} [<header>]}
...
\end{description}
  ...
  The \kwl{http-send-name-header} statement causes the name of the target
...
\subsubsection[ignore-persist]{\kw{ignore-persist} \{ if | unless \} <condition>}
\index{ignore-persist}
  ...
  The \kwl{ignore-persist} statement allows one to declare various ACL-based
  conditions which, when met, will cause a request to ignore persistence.
  This is sometimes useful to load balance requests for static files, which
  oftenly don't require persistence. This can also be used to fully disable
  persistence for a specific User-Agent (for example, some web crawler bots).

  Combined with \kwl{appsession}, it can also help reduce HAProxy memory usage, as
...
See also: \kwl{force-persist}, \kwl{cookie}, and chapter~\ref{chap:using_acl} about ACL usage.

\subsubsection[log global]{\kw{log global}}
\subsubsection[log]{\kw{log} <address> <facility> [<level> [<minlevel>]]}
\subsubsection[no log]{\kw{no log}}
...
\item[<level>]
               is optional and can be specified to filter outgoing messages. By
...
\end{description}
  ...
  \emph{Note:} According to \rfc{3164}, messages are truncated to 1024 bytes before
         being emitted.
...
\subsubsection[log-format]{\kw{log-format} <string>}
\index{log-format}

   Allows you to custom a log line.


See also:~\nameref{subsec:custom_log_format} (\ref{subsec:custom_log_format})

\subsubsection[maxconn]{\kw{maxconn} <conns>}
\index{maxconn}
...
See also: \kwl{server}, global section's \kwl{maxconn}, \kwl{fullconn}

\subsubsection[mode]{\kw{mode} \{ tcp | http | health \}}
\index{mode}
...
See also: \kwl{monitor}, \kwl{monitor-net}

\subsubsection[monitor fail]{\kw{monitor fail} \{ if | unless \} <condition>}
\index{monitor fail}
...
\end{description}
  ...
  haproxy. In this case, one would rely on an ACL involving the \kwl{nbsrv}
  criterion. Note that \kwl{monitor fail} only works in HTTP mode. Both status
  messages may be tweaked using \kwl{errorfile} or \kwl{errorloc} if needed.
...
See also: \kwl{monitor-net}, \kwl{monitor-uri}, \kwl{errorfile}, \kwl{errorloc}

\subsubsection[monitor-net]{\kw{monitor-net} <source>}
\index{monitor-net}
...
\end{description}
  ...
  requests caught by \kwl{monitor-net}.

  Last, please note that only one \kwl{monitor-net} statement can be specified in
...
See also: \kwl{monitor fail}, \kwl{monitor-uri}

\subsubsection[monitor-uri]{\kw{monitor-uri} <uri>}
\index{monitor-uri}
...
See also: \kwl{monitor fail}, \kwl{monitor-net}

\subsubsection[abortonclose]{option \kw{abortonclose}}
...
See also: \kwl{timeout queue} and server's \kwl{maxconn} and \kwl{maxqueue} parameters

\subsubsection[accept-invalid-http-request]{option \kw{accept-invalid-http-request}}
...
\dflb{yes}{yes}{yes}{no}
  ...
  By default, HAProxy complies with \rfc{2616} in terms of message parsing. This
  ...
  list of characters allowed to appear in a URI is well defined by \rfc{3986}, and
  chars 0-31, 32 (space), 34 \chr{"}, 60 \chr{<}, 62 \chr{>}, 92 \chr{\bslash},
  94 \chr{\^}, 96 \chr{`}, 123 \chr{\{}, 124 \chr{|}, 125 \chr{\}}, 127 (delete)
...
See also: \kwl{option accept-invalid-http-response} and \kwl{show errors} on the
             stats socket.

\subsubsection[accept-invalid-http-response]{option \kw{accept-invalid-http-response}}
...
See also: \kwl{option accept-invalid-http-request} and \kwl{show errors} on the
             stats socket.

\subsubsection[allbackups]{option \kw{allbackups}}
...
\dflb{yes}{no}{yes}{yes}
  ...
  at once, because one will not be enough. When \kwl{option allbackups} is enabled,
...
\subsubsection[checkcache]{option \kw{checkcache}}
...
\dflb{yes}{no}{yes}{yes}
  ...
  The \kwl{option checkcache} enables deep inspection of all server responses for
...
\subsubsection[clitcpka]{option \kw{clitcpka}}
...
\dflb{yes}{yes}{yes}{no}
  ...
  Using option \kwl{clitcpka} enables the emission of TCP keep-alive probes on the
...
See also: \kwl{option srvtcpka}, \kwl{option tcpka}

\subsubsection[contstats]{option \kwl{contstats}}
...
\subsubsection[dontlog-normal]{option \kwl{dontlog-normal}}
...
See also: \kwl{log}, \kwl{dontlognull}, \kwl{log-separate-errors} and chapter~\ref{chap:logging} about
             logging.

\subsubsection[dontlognull]{option \kw{dontlognull}}
...
\dflb{yes}{yes}{yes}{no}
  ...
  the logs too much, it is possible to enable option \kwl{dontlognull} to indicate
...
See also: \kwl{log}, \kwl{monitor-net}, \kwl{monitor-uri} and chapter~\ref{chap:logging} about logging.

\subsubsection[forceclose]{option \kw{forceclose}}
...
\dflb{yes}{yes}{yes}{yes}
  ...
  the "Connection: close" set by \kwl{option httpclose}, and if the client does not
  ...
  When this happens, it is possible to use option \kwl{forceclose}. It will
  actively close the outgoing server channel as soon as the server has finished
  to respond. This option implicitly enables the \kwl{httpclose} option. Note that
  ...
  This option may also be combined with \kwl{option http-pretend-keepalive}, which
...
See also: \kwl{option httpclose} and \kwl{option http-pretend-keepalive}

\subsubsection[forwardfor]{option \kw{forwardfor} [ except <network> ] [ header <name> ] [ if-none ]}
\index{forwardfor}
...
\end{description}
  ...
  In order to fix this, ensure that any of the \kwl{httpclose}, \kwl{forceclose} or
  \kwl{http-server-close} options is set when using this option.
...
See also: \kwl{option httpclose}, \kwl{option http-server-close},
             "option forceclose"

\subsubsection[http-no-delay]{option \kw{http-no-delay}}
...
\dflb{yes}{yes}{yes}{yes}
  ...
  When \kwl{option http-no-delay} is present in either the frontend or the backend
...
\subsubsection[http-pretend-keepalive]{option \kw{http-pretend-keepalive}}
...
\dflb{yes}{yes}{yes}{yes}
  ...
  When running with \kwl{option http-server-close} or \kwl{option forceclose}, haproxy
  ...
  By setting \kwl{option http-pretend-keepalive}, haproxy will make the server
  believe it will keep the connection alive. The server will then not fall back
  to the abnormal undesired above. When haproxy gets the whole response, it
  will close the connection with the server just as it would do with the
  \kwl{forceclose} option. That way the client gets a normal response and the
...
See also: \kwl{option forceclose} and \kwl{option http-server-close}

\subsubsection[http-server-close]{option \kw{http-server-close}}
...
\dflb{yes}{yes}{yes}{yes}
  ...
  \kwl{option http-server-close} enables HTTP connection-close mode on the server
  ...
  resources, similarly to \kwl{option forceclose}. It also permits non-keepalive
  capable servers to be served in keep-alive mode to the clients if they
  conform to the requirements of \rfc{2616}. Please note that some servers do not
  ...
  consists in enabling \kwl{option http-pretend-keepalive}.

  At the moment, logs will not indicate whether requests came from the same
  session or not. The accept date reported in the logs corresponds to the end
  of the previous request, and the request time corresponds to the time spent
  waiting for a new request. The keep-alive request time is still bound to the
  timeout defined by \kwl{timeout http-keep-alive} or \kwl{timeout http-request} if
  not set.

  This option may be set both in a frontend and in a backend. It is enabled if
  at least one of the frontend or backend holding a connection has it enabled.
  It is worth noting that \kwl{option forceclose} has precedence over \kwl{option http-server-close}
  and that combining \kwl{http-server-close} with \kwl{httpclose}
  basically achieve the same result as \kwl{forceclose}.

  If this option has been enabled in a "defaults" section, it can be disabled
  in a specific instance by prepending the "no" keyword before it.


See also: \kwl{option forceclose}, \kwl{option http-pretend-keepalive},
             \kwl{httpclose} and section~\ref{sec:http_transaction} "\nameref{sec:http_transaction}".

\subsubsection[http-use-proxy-header]{option \kw{http-use-proxy-header}}
...
\dflb{yes}{yes}{yes}{no}
  ...
  one of the \kwl{httpclose}, \kwl{forceclose} and \kwl{http-server-close} options. Note
...
See also: \kwl{option httpclose}, \kwl{option forceclose} and \kwl{option http-server-close}.


\subsubsection[httpchk]{option \kw{httpchk}}
...
See also: \kwl{option ssl-hello-chk}, \kwl{option smtpchk}, \kwl{option mysql-check},
             "option pgsql-check", "http-check" and the "check", "port" and
             "inter" server options.

\subsubsection[httpclose]{option \kw{httpclose}}
...
See also: \kwl{option forceclose}, \kwl{option http-server-close} and
             "1.1. The HTTP transaction model".

\subsubsection[httplog]{option \kw{httplog} [ clf ]}
\index{httplog}
...
\subsubsection[http\_proxy]{option \kw{http\_proxy}}
...
See also: \kwl{option httpclose}

\subsubsection[independant-streams]{option \kw{independant-streams}}
...
See also: \kwl{timeout client}, \kwl{timeout server} and \kwl{timeout tunnel}

\subsubsection[ldap-check]{option \kw{ldap-check}}
...
See also: \kwl{option httpchk}

\subsubsection[log-health-checks]{option \kw{log-health-checks}}
...
See also: \kwl{log} and chapter~\ref{chap:logging} about logging.

\subsubsection[log-separate-errors]{option \kw{log-separate-errors}}
...
See also: \kwl{log}, \kwl{dontlognull}, \kwl{dontlog-normal} and chapter~\ref{chap:logging} about
             logging.

\subsubsection[logasap]{option \kw{logasap}}
...
See also: \kwl{option httplog}, \kwl{capture response header}, and chapter~\ref{chap:logging} about
             logging.
...
\subsubsection[pgsql-check]{option \kw{pgsql-check} [ user <username> ]}
...
\subsubsection[nolinger]{option \kw{nolinger}}
...
\subsubsection[originalto]{option \kw{originalto} [ except <network> ] [ header <name> ]}
...
\subsubsection[persist]{option \kw{persist}}
...
See also: \kwl{option redispatch}, \kwl{retries}, \kwl{force-persist}

\subsubsection[redispatch]{option \kw{redispatch}}
...
See also: \kwl{redispatch}, \kwl{retries}, \kwl{force-persist}

\subsubsection[redis-check]{option \kw{redis-check}}
...
\subsubsection[smtpchk]{option \kw{smtpchk}}
...
See also: \kwl{option httpchk}, \kwl{source}

\subsubsection[socket-stats]{option \kw{socket-stats}}
...
\subsubsection[splice-auto]{option \kw{splice-auto}}
...
See also: \kwl{option splice-request}, \kwl{option splice-response}, and global
             options "nosplice" and "maxpipes"

\subsubsection[splice-request]{option \kw{splice-request}}
...
\dflb{yes}{yes}{yes}{yes}
  ...
  compile time, and may be globally disabled with the global option \kwl{nosplice}.
...
See also: \kwl{option splice-auto}, \kwl{option splice-response}, and global options
             "nosplice" and "maxpipes"

\subsubsection[splice-response]{option \kw{splice-response}}
...
\dflb{yes}{yes}{yes}{yes}
  ...
  \emph{Important note:} see \kwl{option splice-auto} for usage limitations.
...
See also: \kwl{option splice-auto}, \kwl{option splice-request}, and global options
             \kwl{nosplice} and \kwl{maxpipes}.

\subsubsection[srvtcpka]{option \kw{srvtcpka}}
...
\dflb{yes}{no}{yes}{yes}
  ...
  Using \kwl{option srvtcpka} enables the emission of TCP keep-alive probes on the
...
See also: \kwl{option clitcpka}, \kwl{option tcpka}

\subsubsection[ssl-hello-chk]{option \kw{ssl-hello-chk}}
...
\dflb{yes}{no}{yes}{yes}
  ...
  that it accepts the TCP connection. When \kwl{option ssl-hello-chk} is set, pure
...
\subsubsection[tcp-smart-accept]{option \kw{tcp-smart-accept}}
...
\dflb{yes}{yes}{yes}{no}
  ...
  fall back to normal behavior by specifying \kwl{no option tcp-smart-accept}.

  It is also possible to force it for non-HTTP proxies by simply specifying
  \kwl{option tcp-smart-accept}. For instance, it can make sense with some services
...
See also: \kwl{option tcp-smart-connect}

\subsubsection[tcp-smart-accept]{option \kw{tcp-smart-connect}}
...
\dflb{yes}{no}{yes}{yes}
  ...
  This feature is enabled when \kwl{option tcp-smart-connect} is set in a backend.
...
See also: \kwl{option tcp-smart-accept}

\subsubsection[tcpka]{option \kw{tcpka}}
\index{tcpka}
...
\dflb{yes}{yes}{yes}{yes}
  ...
  Using option \kwl{tcpka} enables the emission of TCP keep-alive probes on both
  the client and server sides of a connection. Note that this is meaningful
  only in "defaults" or "listen" sections. If this option is used in a
  frontend, only the client side will get keep-alives, and if this option is
  used in a backend, only the server side will get keep-alives. For this
  reason, it is strongly recommended to explicitly use \kwl{option clitcpka} and
  \kwl{option srvtcpka} when the configuration is split between frontends and
  backends.


See also: \kwl{option clitcpka}, \kwl{option srvtcpka}

\subsubsection[tcplog]{option \kw{tcplog}}
...
\dflb{yes}{yes}{yes}{yes}
  ...
  By default, the log output format is very poor, as it only contains the
  source and destination addresses, and the instance name. By specifying
  \kwl{option tcplog}, each log line turns into a much richer format including, but
  not limited to, the connection timers, the session status, the connections
  numbers, the frontend, backend and server name, and of course the source
  address and ports. This option is useful for pure TCP proxies in order to
  find which of the client or server disconnects or times out. For normal HTTP
  proxies, it's better to use \kwl{option httplog} which is even more complete.

  This option may be set either in the frontend or the backend.


See also: \kwl{option httplog}, andchapter~\ref{chap:logging} about logging.

\subsubsection[transparent]{option \kw{transparent}}
...
See also: the \kwl{usesrc} argument of the \kwl{source} keyword, and the
            \kwl{transparent} option of the \kwl{bind} keyword.

\subsubsection[persist rdp-cookie]{\kw{persist rdp-cookie}}
...
\dflb{yes}{no}{yes}{yes}
  ...
  which is still UP (or if \kwl{option persist} is set), then the connection is
  forwarded to this server.

  Note that this only makes sense in a TCP backend, but for this to work, the
  frontend must have waited long enough to ensure that an RDP cookie is present
  in the request buffer. This is the same requirement as with the \kwl{rdp-cookie}
...
See also: \kwl{balance rdp-cookie}, \kwl{tcp-request}, the \kwl{req\_rdp\_cookie} ACL and
  the rdp\_cookie pattern fetch function.

\subsubsection[rate-limit sessions]{\kw{rate-limit sessions} <rate>}
...
\dflb{yes}{yes}{yes}{no}
  ...
  sense to increase the socket's backlog using the \kwl{backlog} keyword.
  ...
  \emph{Note:} when the maximum rate is reached, the frontend's status is not changed
         ...
         \kwl{socket-stats} option is enabled.

See also: the \kwl{backlog} keyword and the \kwl{fe\_sess\_rate} ACL criterion.

\subsubsection[redirect location] {\kw{redirect location} <to> [code <code>] <option> [{if | unless} <condition>]}
\subsubsection[redirect prefix] {\kw{redirect prefix} <to> [code <code>] <option> [{if | unless} <condition>]}
...
\dflb{no}{yes}{yes}{yes}
  ...
  \begin{description}
  \item[<to>]
              With \kwl{redirect location}, the exact value in <to> is placed into
              the HTTP "Location" header. In case of \kwl{redirect prefix}, the
...
\subsubsection[redisp]{\kw{redisp} (deprecated)}
\subsubsection[redispatch]{\kw{redispatch} (deprecated)}
\index{redisp}
\index{redispatch}
...
\dflb{yes}{no}{yes}{yes}
  ...
  Specifying \kwl{redispatch} will allow the proxy to break their persistence and
  ...
  \kwl{option redispatch} instead.


See also: \kwl{option redispatch}

\subsubsection[reqadd]{\kw{reqadd}  <string> [ \{if | unless\} <cond> ]}
...
See also: \kwl{rspadd}, chapter~\ref{chap:header_manipulation} about HTTP header manipulation,
            and chapter~\ref{chap:using_acl} about ACLs.

\subsubsection[reqallow]{\kw{reqallow}  <search> [ \{if | unless\} <cond> ]}
\subsubsection[reqiallow]{\kw{reqiallow} <search> [ \{if | unless\} <cond> ] (ignore case)}
...
See also: \kwl{reqdeny}, \kwl{block}, chapter~\ref{chap:header_manipulation} about HTTP header manipulation, and
...
See also: \kwl{reqadd}, \kwl{reqrep}, \kwl{rspdel}, section 6 about HTTP header
...
See also: \kwl{reqallow}, \kwl{rspdeny}, \kwl{block}, chapter~\ref{chap:header_manipulation} about HTTP header
            manipulation, and chapter~\ref{chap:using_acl} about ACLs.
...
See also: \kwl{reqallow}, \kwl{reqdeny}, \kwl{block}, chapter~\ref{chap:header_manipulation} about HTTP header
            manipulation, and chapter~\ref{chap:using_acl} about ACLs.
...
See also: \kwl{reqadd}, \kwl{reqdel}, \kwl{rsprep}, chapter~\ref{chap:header_manipulation} about HTTP header
            manipulation, and chapter~\ref{chap:using_acl} about ACLs.
...
See also: \kwl{reqallow}, \kwl{reqdeny}, \kwl{reqpass}, chapter~\ref{chap:header_manipulation} about HTTP header
            manipulation, and chapter~\ref{chap:using_acl} about ACLs.

\subsubsection[retries]{\kw{retries} <value>}
\index{retries}
...
\dflb{yes}{no}{yes}{yes}
  ...
  When \kwl{option redispatch} is set, the last retry may be performed on another
  server even if a cookie references a different server.


See also: \kwl{option redispatch}

\subsubsection[rspadd]{\kw{rspadd} <string> [ \{if | unless\} <cond>] }
\index{rspadd}
...
See also: \kwl{reqadd}, chapter~\ref{chap:header_manipulation} about HTTP header manipulation,
  and chapter~\ref{chap:using_acl} about ACLs.

\subsubsection[rspdel]{\kw{rspdel} <search> [ \{if | unless\} <cond> ]}
\subsubsection[rspidel]{\kw{rspidel} <search> [ \{if | unless\} <cond> ]  (ignore case)}
\index{rspdel}
\index{rspidel}
...
\dflb{no}{yes}{yes}{yes}
  ...
  \item[<search>] is the regular expression applied to HTTP headers and to the
              ...
              The \kwl{rspdel} keyword strictly matches case while \kwl{rspidel}
              ignores case.
...
See also: \kwl{rspadd}, \kwl{rsprep}, \kwl{reqdel}, chapter~\ref{chap:header_manipulation} about HTTP header
            manipulation, and chapter~\ref{chap:using_acl} about ACLs.

\subsubsection[rspdeny] {\kw{rspdeny} <search> [ \{if | unless\} <cond> ]}
\subsubsection[rspideny] {\kw{rspideny} <search> [ \{if | unless\} <cond> ] (ignore case)}
\index{rspdeny}
\index{rspideny}
...
\dflb{no}{yes}{yes}{yes}
  ...
  \item[<search>] is the regular expression applied to HTTP headers and to the
              ...
              is required. Any space or known delimiter must be escaped using
              a backslash \chr{\bslash}. The pattern applies to a full line at a time.
              The \kwl{rspdeny} keyword strictly matches case while \kwl{rspideny}
              ignores case.
...
See also: \kwl{reqdeny}, \kwl{acl}, \kwl{block}, chapter~\ref{chap:header_manipulation} about HTTP header manipulation
            and chapter~\ref{chap:using_acl} about ACLs.

\subsubsection[rsprep]{\kw{rsprep} <search> <string> [ \{if | unless\} <cond> ]}
\subsubsection[rspirep]{\kw{rspirep} <search> <string> [ \{if | unless\} <cond> ]  (ignore case)}
\index{rsprep}
\index{rspirep}
...
\dflb{no}{yes}{yes}{yes}
  ...
  \item[<search>] is the regular expression applied to HTTP headers and to the
              ...
              is required. Any space or known delimiter must be escaped using
              a backslash \chr{\bslash}. The pattern applies to a full line at a time.
              The \kwl{rsprep} keyword strictly matches case while \kwl{rspirep}
              ignores case.

  \item[<string>] is the complete line to be added. Any space or known delimiter
              must be escaped using a backslash \chr{\bslash}. References to matched
              pattern groups are possible using the common \chr{\bslash{}N} form, with N
              being a single digit between 0 and 9. Please refer to section
              6 about HTTP header manipulation for more information.
...
See also: \kwl{rspadd}, \kwl{rspdel}, \kwl{reqrep}, chapter~\ref{chap:header_manipulation}
...
\dflb{no}{no}{yes}{yes}
  ...
  \item[<name>] is the internal name assigned to this server. This name will
              appear in logs and alerts.  If \kwl{http-send-server-name} is
...
See also: \kwl{default-server}, \kwl{http-send-name-header} and chapter~\ref{chap:server_options} about
             server options

\subsubsection[source]{\kw{source} <addr>[:<port>] [usesrc \{ <addr2>[:<port2>] | client | clientip \} ]}
...
\dflb{yes}{no}{yes}{yes}
  ...
  \item[<occ>] is the occurrence number of a value to be used in a multi-value
              header. This is to be used in conjunction with \kwl{hdr\_ip}(<hdr>)",
  ...
  The \kwl{source} keyword is useful in complex environments where a specific
...
See also: the \kwl{source} server option in chapter~\ref{chap:server_options}, the Tproxy patches for
             the Linux kernel on \site{www.balabit.com}, the \kwl{bind} keyword.

\subsubsection[srvtimeout]{\kw{srvtimeout} <timeout> (deprecated)}
\index{srvtimeout}
...
See also: \kwl{timeout server}, \kwl{timeout tunnel}, \kwl{timeout client} and
             \kwl{clitimeout}.

\subsubsection[stats admin]{\kw{stats admin} \{ if | unless \} <cond>}
\index{stats admin}
...
See also: \kwl{stats enable}, \kwl{stats auth}, \kwl{stats http-request}, \kwl{nbproc},
            \kwl{bind-process}, section~\ref{sec:userlists} about userlists and chapter~\ref{chap:using_acl} about
             ACL usage.

\subsubsection[stats auth]{\kw{stats auth} <user>:<passwd>}
\index{stats auth}
...
See also: \kwl{stats enable}, \kwl{stats realm}, \kwl{stats scope}, \kwl{stats uri}

\subsubsection[stats enable]{\kw{stats enable}}
...
See also: \kwl{stats auth}, \kwl{stats realm}, \kwl{stats uri}


\subsubsection[stats hide-version]{\kw{stats hide-version}}
\index{stats hide-version}
  ...
  target known weaknesses with specific attacks. The \kwl{stats hide-version}
...
See also: \kwl{stats auth}, \kwl{stats enable}, \kwl{stats realm}, \kwl{stats uri}

\subsubsection[stats http-request]{\kw{stats http-request} \{ allow | deny | auth [realm <realm>] \} [ \{ if | unless \} <condition> ]}
\index{stats http-request}

  Access control for statistics

\dflb{no}{no}{yes}{yes}

  As \kwl{http-request}, these set of options allow to fine control access to
...
See also: \kwl{http-request}, section~\ref{sec:userlists} about userlists and chapter~\ref{chap:using_acl}
             about ACL usage.

\subsubsection[stats realm]{\kw{stats realm} <realm>}
...
\dflb{yes}{no}{yes}{yes}
  ...
  This statement is useful only in conjunction with \kwl{stats auth} since it is
...
See also: \kwl{stats auth}, \kwl{stats enable}, \kwl{stats uri}

\subsubsection[stats refresh]{\kwl{stats refresh} <delay>}
\index{stats refresh}
...
\subsubsection[stats scope]{\kw{stats scope} \{ <name> | "." \}}
\index{stats scope}
...
\subsubsection[stats show-desc]{\kw{stats show-desc} [ <desc> ]}

\index{stats show-desc}
...
See also: \kwl{show-node}, \kwl{stats enable}, \kwl{stats uri} and \kwl{description} in
            global section.

\subsubsection[show-legends]{stats \kw{show-legends}}

\index{stats show-legends}
...
See also: \kwl{stats enable}, \kwl{stats uri}.


\subsubsection[stats show-node]{\kw{stats show-node} [ <name> ]}
\index{stats show-node}
...
See also: \kwl{show-desc}, \kwl{stats enable}, \kwl{stats uri}, and \kwl{node} in global
            section.

\subsubsection[stats uri]{\kw{stats uri} <prefix>}
...
\dflb{yes}{no}{yes}{yes}
  ...
  \item[<prefix>] is the prefix of any URI which will be redirected to stats. This
              prefix may contain a question mark \chr{?} to indicate part of a
              query string.
  \end{description}
  ...
  The default URI compiled in haproxy is \site{/haproxy?stats}, but this may be
...
See also: \kwl{stats auth}, \kwl{stats enable}, \kwl{stats realm}

\subsubsection[stick match]{\kw{stick match} <pattern> [table <table>] [ \{if | unless\} <cond>]}
\index{stick match}
...
\dflb{no}{no}{yes}{yes}
  ...
  The table has to be declared using the \kwl{stick-table} statement. It must be of
...
See also: \kwl{stick-table}, \kwl{stick on}, \kwl{nbproc}, \kwl{bind-process} and chapter~\ref{chap:using_acl}
             about ACLs and pattern extraction.


\subsubsection[stick on]{\kw{stick on} <pattern> [table <table>] [ \{if | unless\} <condition> ]}
\index{stick on}
...
\dflb{no}{no}{yes}{yes}
  ...
  \emph{Note:} This form is exactly equivalent to \kwl{stick match} followed by
         \kwl{stick store-request}, all with the same arguments. Please refer
...
See also: \kwl{stick match}, \kwl{stick store-request}, \kwl{nbproc} and \kwl{bind-process}.


\subsubsection[stick store-request]{\kw{stick store-request} <pattern> [table <table>] [ \{if | unless\} <condition> ]}
\index{stick store-request}
...
\dflb{no}{no}{yes}{yes}
  ...
  Some protocols or applications require complex stickiness rules and cannot
  always simply rely on cookies nor hashing. The \kwl{stick store-request} statement
  ...
  match it using the \kwl{stick match} statement. Obviously the extracted part must
  ...
  a type compatible with the pattern. By default it is the one which is present
  in the same backend. It is possible to share a table with other backends by
  referencing it using the \kwl{table} keyword. If another table is referenced,
  the server's ID inside the backends are used. By default, all server IDs
  start at 1 in each backend, so the server ordering is enough. But in case of
  doubt, it is highly recommended to force server IDs using their "id" setting.

  It is possible to restrict the conditions where a \kwl{stick store-request}
  ...
  There is no limit on the number of \kwl{stick store-request} statements, but
  ...
  The \kwl{store-request} rules are evaluated once the server connection has been
...
\subsubsection[stick-table type]{\kw{stick-table type} \{ ip | integer | string [len <length>] | binary [len <length>] \} size <size> [expire <expire>] [nopurge] [peers <peersect>] [store <data\_type>]*}
\index{stick-table type}
...
\dflb{no}{yes}{yes}{yes}
  ...
  \item[-] server\_id: this is an integer which holds the numeric ID of the server a
      request was assigned to. It is used by the \kwl{stick match}, \kwl{stick store},
...
See also: \kwl{stick match}, \kwl{stick on}, \kwl{stick store-request}, section~\ref{sec:time_format}
             about time format and chapter~\ref{chap:using_acl} about ACLs.

\subsubsection[stick store-response]{\kw{stick store-response} <pattern> [table <table>] [ \{if | unless\} <condition> ]}
\index{stick store-response}
...
\dflb{no}{no}{yes}{yes}
  ...
  It is possible to restrict the conditions where a \kwl{stick store-response}
  statement will apply, using "if" or "unless" followed by a condition. This
  ...
  There is no limit on the number of \kwl{stick store-response} statements, but
  there is a limit of 8 simultaneous stores per request or response. This
  makes it possible to store up to 8 criteria, all extracted from either the
  request or the response, regardless of the number of rules. Only the 8 first
  ones which match will be kept. Using this, it is possible to feed multiple
...
See also: \kwl{stick-table}, \kwl{stick on}, and chapter~\ref{chap:using_acl} about ACLs and pattern
             extraction.

\subsubsection[tcp-request connection]{\kw{tcp-request connection} <action> [ \{if | unless\} <condition> ]}
\index{tcp-request connection}
...
See also: \kwl{tcp-request content}, \kwl{stick-table}

\subsubsection[tcp-request content]{\kw{tcp-request content} <action> [ \{if | unless\} <condition> ]}
\index{tcp-request content}
...
See also: \kwl{tcp-request connection}, \kwl{tcp-request inspect-delay}

\subsubsection[tcp-request inspect-delay]{\kw{tcp-request} inspect-delay <timeout>}
\index{tcp-request inspect-delay}
...
See also: \kwl{tcp-request content accept}, \kwl{tcp-request content reject},
            \kwl{timeout client}.

\subsubsection[tcp-response content]{\kw{tcp-response content} <action> [ \{if | unless\} <condition> ]}
\index{tcp-response content}
...
See also: \kwl{tcp-request content}, \kwl{tcp-response inspect-delay}

\subsubsection[tcp-response]{\kw{tcp-response inspect-delay} <timeout>}
...
See also: \kwl{tcp-response content}, \kwl{tcp-request inspect-delay}.

\subsubsection[timeout check]{\kw{timeout check} <timeout>}
\index{timeout check}
...
See also: \kwl{timeout connect}, \kwl{timeout queue}, \kwl{timeout server},
            \kwl{timeout tarpit"}.

\subsubsection[timeout client]{\kw{timeout client} <timeout>}
\subsubsection[timeout clitimeout]{\kw{timeout clitimeout} <timeout> (deprecated)}

\index{timeout client}
\index{timeout clitimeout}
...
See also: \kwl{clitimeout}, \kwl{timeout server}, \kwl{timeout tunnel}.

\subsubsection[timeout connect]{\kw{timeout connect} <timeout>}
\subsubsection[timeout contimeout]{\kw{timeout connect} <timeout> (deprecated)}
\index{timeout contimeout}
\index{timeout connect}
...
See also: \kwl{timeout check}, \kwl{timeout queue}, \kwl{timeout server}, \kwl{contimeout},
            "timeout tarpit".

\subsubsection[timeout http-keep-alive]{\kw{timeout http-keep-alive} <timeout>}
\index{timeout http-keep-alive}
...
See also: \kwl{timeout http-request}, \kwl{timeout client}.

\subsubsection[timeout http-request]{\kw{timeout http-request} <timeout>}
\index{timeout http-request}
...
See also: \kwl{timeout http-keep-alive}, \kwl{timeout client}.

\subsubsection[timeout queue]{\kw{timeout queue} <timeout>}
...
See also: \kwl{timeout connect}, \kwl{contimeout}.

\subsubsection[timeout server]{\kw{timeout server} <timeout>}
\subsubsection[timeout srvtimeout]{\kw{timeout srvtimeout} <timeout> (deprecated)}
\index{timeout server}
\index{timeout srvtimeout}
...
See also: \kwl{srvtimeout}, \kwl{timeout client} and \kwl{timeout tunnel}.

\subsubsection[timeout tarpit]{\kw{timeout tarpit} <timeout>}
\index{timeout tarpit}
...
\subsubsection[timeout tunnel]{\kw{timeout tunnel} <timeout>}
\index{timeout tunnel}
...
See also: \kwl{timeout client}, \kwl{timeout server}.

\subsubsection[transparent]{\kw{transparent} (deprecated)}
\index{transparent}
...
See also: \kwl{option transparent}

\subsubsection[unique-id-format]{\kw{unique-id-format} <string>}
\index{unique-id-format}
...
See also: \kwl{unique-id-header}

\subsubsection[unique-id-header]{\kw{unique-id-header} <name>}
\index{unique-id-header}
...
See also: \kwl{unique-id-format}

\subsubsection[use\_backend]{\kw{use\_backend} <backend> \{ if | unless \} <condition>}
...
\dflb{no}{yes}{yes}{no}
  ...
  There may be as many "use\_backend" rules as desired. All of these rules are
...
See also: \kwl{default\_backend}, \kwl{tcp-request}, and chapter~\ref{chap:using_acl} about ACLs.

\subsubsection[use-server]{\kw{use-server} <server> \{ if | unless \} <condition>}
\index{use-server}
...
See also: \kwl{use\_backend}, chapter~\ref{chap:server_options} about server and chapter~\ref{chap:using_acl} about ACLs.
