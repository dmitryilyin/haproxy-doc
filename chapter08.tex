...
The TCP format is used when option \kwl{tcplog} is specified in the frontend, and
...
emitted earlier if option \kwl{logasap} is specified, which makes sense in most
...
specifying option \kwl{dontlognull} in the frontend. Successful connections will
not be logged if option \kwl{dontlog-normal} is specified in the frontend. A few
...
\item[-]
   "Tt" is the total time in milliseconds elapsed between the accept and the
    ...
    option \kwl{logasap} was specified, then the time counting stops at the moment
...
\item[-]
    ...
    the client when the log is emitted. If option \kwl{logasap} is specified, the
...
\item[-]
    ...
    "~\nameref{chap:global_parameters}" to find how to tune the system.
...
is enabled by when option \kwl{httplog} is specified in the frontend. It provides
the same level of information as the TCP format with additional features which
are specific to the HTTP protocol. Just like the TCP format, the log is usually
emitted at the end of the session, unless option \kwl{logasap} is specified, which
generally only makes sense for download sites. A session which matches the
"monitor" rules will never logged. It is also possible not to log sessions for
which no data were sent by the client by specifying option \kwl{dontlognull} in the
frontend. Successful connections will not be logged if option \kwl{dontlog-normal}
...
\item[-]
    ...
    the log is emitted. This does include HTTP headers. If option \kwl{logasap} is
...
\item[-]
    ...
    "~\nameref{chap:global_parameters}" to find how to tune the
    system.
...
\item[-]
    ...
    gone through the global queue. It makes it possible to estimate the average
    queue length, which easily translates into a number of missing servers when
    divided by a server's \kwl{maxconn} parameter. It is worth noting that if a
    session experiences a redispatch, it may pass twice in the backend's queue,
    and then both positions will be cumulated. A request should not pass
    through both the server queue and the backend queue unless a redispatch
    occurs.
...
\item[-]
    ...
    setting option \kwl{dontlognull} in the frontend. It also disables logging of
    port scans, which may or may not be desired.
\item[-]
    if the connection come from a known source network, use \kwl{monitor-net} to
...
\item[-]
    if the tests are performed on a known URI, use \kwl{monitor-uri} to declare
...
option \kwl{logasap} in the frontend. Haproxy will then log as soon as possible,
...
\kwl{log-separate-errors} is used, connections which experience errors, timeouts,
...
\kwl{dontlog-normal} is set on the frontend, all normal connections will not be
...
the session termination flags. In TCP mode with option \kwl{tcplog} set on the
...
\item[-]
    ...
    and the moment both ends were closed. The exception is when the \kwl{logasap}
...
\item[-]
    ...
    to solve this issue, it will be needed to specify option \kwl{httpclose} on
    ...
    close. Then it will be required to use option \kwl{forceclose}. Having the
    smallest possible 'Tt' is important when connection regulation is used with
    the \kwl{maxconn} option on the servers, since no new connection will be sent
...
\begin{description}
\item[Tq/Tw/Tc/Tr/+Tt]
                   The option \kwl{logasap} is present on the frontend and the log
...
\item[-1/xx/xx/xx/Tt]
                   ...
                   then \kwl{timeout http-request} and \kwl{timeout client} settings.
...
\item[Tq/Tw/-1/xx/Tt]
                   ...
                   \kwl{timeout connect} setting. Note that the tarpit action might
...
\item[Tq/Tw/Tc/-1/Tt]
                   ...
                   termination flags, then check the \kwl{timeout server} setting.
\end{description}
...
\item
     on the second character, the TCP or HTTP session state when it was closed:
        ...
        \item[Q:]
            ...
            only happen when servers have a \kwl{maxconn} parameter set. It can
...
\item
    ...
    the client (only in HTTP mode):
        ...
        \item[E:]
            ...
            older than what is allowed by the \kwl{maxidle} cookie parameter, so
        ...
        \item[O:]
            ...
            older than what is allowed by the \kwl{maxlife} cookie parameter, so
...
\item[-]
    ...
    cookie returned by the server (only in HTTP mode) :
        ...
        \item[U:]
            ...
            the client. This can only happen in insert mode with \kwl{maxidle}. It
        ...
        \item[R:]
            ...
            happens in \kwl{cookie rewrite} or \kwl{cookie prefix} modes.
        \item[D:]
...
\begin{description}
     ...
     \item[CC]   
          ...
          waiting for the server to respond or for \kwl{timeout connect} to expire.
     \item[CD]
     ...
     \item[cD]
          ...
          \kwl{timeout client} delay. This is often caused by network failures on
     ...
     \item[cH]
          The \kwl{timeout client} stroke while waiting for client data during a
     ...
     \item[CT]
          ...
          might make sense to lower the \kwl{timeout tarpit} value to something
     ...
     \item[sC]
          The \kwl{timeout connect} stroke before a connection to the server could
     ...
     \item[sD]
          ...
          \kwl{timeout server} setting during the data phase. This is often caused
     ...
     \item[sH]
          The \kwl{timeout server} stroke before the server could return its
          response headers. This is the most common anomaly, indicating too
          long transactions, probably caused by server or database saturation.
          The immediate workaround consists in increasing the \kwl{timeout server}
          setting, but it is important to keep in mind that the user experience
          will suffer from these long response times. The only long term
          solution is to fix the application.
     \item[sQ]
          The session spent too much time in queue and has been expired. See
          the \kwl{timeout queue} and \kwl{timeout connect} settings to find out how to
     ...
     \item[PC]
          ...
          The global \kwl{maxconn} parameter may be increased in the configuration
          so that it does not happen anymore. This status is very rare and
          might happen when the global \kwl{ulimit-n} parameter is forced by hand.
     \item[PD]
...
\begin{description}
     ...
     \item[II]
          ...
          a \kwl{server} entry is removed from the configuration, since its cookie
     ...
     \item[VU]
          ...
          there was a date but the \kwl{maxidle} parameter was not set, so that the
          cookie can be switched to unlimited time.
     \item[EI]
          A cookie was provided by the client, with a last visit date which is
          too old for the \kwl{maxidle} parameter, so the cookie was ignored and a
          new cookie was inserted in the response.
     \item[OI]
          A cookie was provided by the client, with a first visit date which is
          too old for the \kwl{maxlife} parameter, so the cookie was ignored and a
          new cookie was inserted in the response.
     \item[DI]
...
escape character \chr{\#} itself is also encoded to avoid any ambiguity \chr{\#23}. It
is the same for the character \chr{"} which becomes \chr{\#22}, as well as \chr{\{}, \chr{|} and
...
the quote \chr{"} with a backslash \chr{\bslash}. The reverse operation can safely be
